...
function generateSnowflake(timestampMs: bigint, workerId: bigint, processId: bigint, increment: bigint): string {
	const timestamp = timestampMs - DISCORD_EPOCH;
	const value = (timestamp << 22n) | (workerId << 17n) | (processId << 12n) | increment;
